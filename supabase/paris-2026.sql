-- Section « Découvrir Paris » : intégration du guide des sites touristiques
-- fourni par l'organisation (Département des Arts et de la Culture de la Diaspora).
-- 4 catégories, 16 sites, avec adresses, transports, tarifs et photos.
--
-- À coller dans Supabase ▸ SQL Editor. Écrase la section « paris » du contenu.
-- Réexécutable sans risque. Personnalisable ensuite depuis /admin ▸ Découvrir Paris.

update public.content set data = '{
  "transport": {
    "line1": {
...
}'::jsonb, updated_at = now() where section = 'paris';
...
select 'paris', '{
 ...
 where not exists (select 1 from public.content where section = 'paris');

-- Contrôle : doit renvoyer 4 catégories.
select jsonb_array_length(data->'categories') as nb_categories from public.content where section = 'paris';
